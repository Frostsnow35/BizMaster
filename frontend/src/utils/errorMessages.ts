...
const ERROR_MAPPINGS: ErrorMapping[] = [
  ...
  {
    pattern: /缺少日期列|无法进行趋势预测|无法预测|聚合后无可用数据点|预测/,
    title: '暂时无法预测',
    description: '无法对当前数据源进行趋势预测，通常是因为缺少日期字段或有效数据点不足。',
    suggestions: [
      '在"数据管理"中确认该数据源包含日期列（如"下单时间""支付时间"）',
      '确认需要预测的指标列存在且为数值格式',
      '如有必要，前往"数据管理"→"字段映射"手动指定日期与指标字段',
    ],
  },
  {
    pattern: /数据源不存在|报告不存在|定时任务不存在/,
    title: '数据不存在',
    description: '您访问的数据源、报告或定时任务已不存在，可能已被删除。',
    suggestions: [
      '返回列表页刷新后重试',
      '如数据被误删，请重新上传或重新创建对应任务',
    ],
  },
]
